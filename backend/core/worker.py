...
def process_chunk(chunk_data):
    ...
    chunk_df contains 'Tên hàng raw' and optionally 'Mã HS'.
    ...
    has_hs = 'Mã HS' in chunk_df.columns
    ...
    if has_hs:
        hs_series = chunk_df['Mã HS'].fillna('').astype(str)
        dict_predictions = input_for_ai.combine(hs_series, lambda text, hs: matcher.predict(text, hs_code=hs if hs else None))
    else:
